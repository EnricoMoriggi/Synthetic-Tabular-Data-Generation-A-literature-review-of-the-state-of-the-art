...
Original file is located at
    https://colab.research.google.com/drive/1lOLJnvXII4gJklpJHSIBEZVTeJQM61od
"""
...
df['Geography']=df['Geography'].replace({'France':0,'Spain':1,'Germany':2}).astype(float)
...
columns = ['CreditScore', 'Geography', 'Age', 'Tenure', 'Balance',
...
for i, col in enumerate(columns, 1):
    plt.subplot(3, 3, i)
    sns.boxplot(x='Exited', y=col, data=df,  palette={0: 'lawngreen', 1: 'red'},
                hue='Exited')
...
df_CTGAN = ctgan.sample(9998)
print(df_CTGAN.head(50))

df_CTGAN['Geography']=df_CTGAN['Geography'].round().abs()
df_CTGAN['Tenure']=df_CTGAN['Tenure'].round().abs()
df_CTGAN['Exited']=df_CTGAN['Exited'].round().abs()
df_CTGAN['NumOfProducts']=df_CTGAN['NumOfProducts'].round().abs()
df_CTGAN['IsActiveMember']=df_CTGAN['IsActiveMember'].round().abs()
df_CTGAN['HasCrCard']=df_CTGAN['HasCrCard'].round().abs()
...
df_TVAE= tvae.sample(9998)
print(df_TVAE.head(10))

df_TVAE['Geography']=df_TVAE['Geography'].round().abs()
df_TVAE['Tenure']=df_TVAE['Tenure'].round().abs()
df_TVAE['Exited']=df_TVAE['Exited'].round().abs()
df_TVAE['NumOfProducts']=df_TVAE['NumOfProducts'].round().abs()
df_TVAE['IsActiveMember']=df_TVAE['IsActiveMember'].round().abs()
df_TVAE['HasCrCard']=df_TVAE['HasCrCard'].round().abs()
...
z = Variable(Tensor(np.random.normal(0, 1, (9998, latent_dim))))
...
df_GAN['Geography']=df_GAN['Geography'].round().abs()
df_GAN['Tenure']=df_GAN['Tenure'].round().abs()
df_GAN['Exited']=df_GAN['Exited'].round().abs()
df_GAN['NumOfProducts']=df_GAN['NumOfProducts'].round().abs()
df_GAN['IsActiveMember']=df_GAN['IsActiveMember'].round().abs()
df_GAN['HasCrCard']=df_GAN['HasCrCard'].round().abs()
...
df_DCGAN['Geography']=df_DCGAN['Geography'].round().abs()
df_DCGAN['Tenure']=df_DCGAN['Tenure'].round().abs()
df_DCGAN['Exited']=df_DCGAN['Exited'].round().abs()
df_DCGAN['NumOfProducts']=df_DCGAN['NumOfProducts'].round().abs()
df_DCGAN['IsActiveMember']=df_DCGAN['IsActiveMember'].round().abs()
df_DCGAN['HasCrCard']=df_DCGAN['HasCrCard'].round().abs()
...
vae.eval()
with torch.no_grad():
    z = torch.randn(9998, embedding_dim)
...
df_VAE['Geography']=df_VAE['Geography'].round().abs()
df_VAE['Tenure']=df_VAE['Tenure'].round().abs()
df_VAE['Exited']=df_VAE['Exited'].round().abs()
df_VAE['NumOfProducts']=df_VAE['NumOfProducts'].round().abs()
df_VAE['IsActiveMember']=df_VAE['IsActiveMember'].round().abs()
df_VAE['HasCrCard']=df_VAE['HasCrCard'].round().abs()
...
samples = kde.sample(9998)
samples_denorm = scaler.inverse_transform(samples)
df_KDE = pd.DataFrame(samples_denorm, columns=data.columns)

print(df_KDE.head())

df_KDE['Geography']=df_KDE['Geography'].round().abs()
df_KDE['Tenure']=df_KDE['Tenure'].round().abs()
df_KDE['Exited']=df_KDE['Exited'].round().abs()
df_KDE['NumOfProducts']=df_KDE['NumOfProducts'].round().abs()
df_KDE['IsActiveMember']=df_KDE['IsActiveMember'].round().abs()
df_KDE['HasCrCard']=df_KDE['HasCrCard'].round().abs()

print(df_KDE.head(50))
...
samples, _ = gmm.sample(9998)
samples_denorm = scaler.inverse_transform(samples)
df_GMM = pd.DataFrame(samples_denorm, columns=data.columns)

print(df_GMM.head())

df_GMM['Geography']=df_GMM['Geography'].round().abs()
df_GMM['Tenure']=df_GMM['Tenure'].round().abs()
df_GMM['Exited']=df_GMM['Exited'].round().abs()
df_GMM['NumOfProducts']=df_GMM['NumOfProducts'].round().abs()
df_GMM['IsActiveMember']=df_GMM['IsActiveMember'].round().abs()
df_GMM['HasCrCard']=df_GMM['HasCrCard'].round().abs()
...
df_CTGAN['Exited'] = df_CTGAN['Exited'].astype(int)
df_DCGAN['Exited'] =  df_DCGAN['Exited'].astype(int)
df_VAE['Exited'] = df_VAE['Exited'].astype(int)
df_KDE['Exited'] = df_KDE['Exited'].astype(int)
df_GMM['Exited'] = df_GMM['Exited'].astype(int)
df_GAN['Exited'] = df_GAN['Exited'].astype(int)
df_TVAE['Exited'] = df_TVAE['Exited'].astype(int)

X_train, X_test, y_train, y_test = train_test_split(df.drop(columns=['Exited']), df['Exited'], test_size=0.2, random_state=42)
X_train_ctgan, X_test_ctgan, y_train_ctgan, y_test_ctgan = train_test_split(df_CTGAN.drop(columns=['Exited']), df_CTGAN['Exited'], test_size=0.2, random_state=42)
X_train_dcgan, X_test_dcgan, y_train_dcgan, y_test_dcgan = train_test_split(df_DCGAN.drop(columns=['Exited']), df_DCGAN['Exited'], test_size=0.2, random_state=42)
X_train_vae, X_test_vae, y_train_vae, y_test_vae = train_test_split(df_VAE.drop(columns=['Exited']), df_VAE['Exited'], test_size=0.2, random_state=42)
X_train_kde, X_test_kde, y_train_kde, y_test_kde = train_test_split(df_KDE.drop(columns=['Exited']), df_KDE['Exited'], test_size=0.2, random_state=42)
X_train_gmm, X_test_gmm, y_train_gmm, y_test_gmm = train_test_split(df_GMM.drop(columns=['Exited']), df_GMM['Exited'], test_size=0.2, random_state=42)
X_train_gan, X_test_gan, y_train_gan, y_test_gan = train_test_split(df_GAN.drop(columns=['Exited']), df_GAN['Exited'], test_size=0.2, random_state=42)
X_train_tvae, X_test_tvae, y_train_tvae, y_test_tvae = train_test_split(df_TVAE.drop(columns=['Exited']), df_TVAE['Exited'], test_size=0.2, random_state=42)
